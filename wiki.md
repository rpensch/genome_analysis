...
The analyses that were carried out in the course of the projects are described below together with a short overview and discussion of the results. Questions from the Student Manual are answered either directly in the flow text or as a seperate point below the corresponding analyses. Results and graphics are referred to in the text are in the [results folder](https://github.com/rpensch/genome_analysis/tree/master/results). 
...
The quality scores are encoded with ASCII. This means that the numeric quality scores are stored in the FASTQ file as letters. For paired reads, there should be two files available (R1 and R2) with the same header. 
...
The k-mer size greatly affects the structure of the graph. A small k-mer size can lead to a very branched graph because repeats and duplications due to errors in reads that are longer than the k-mer size are not recognized and k-mers are wrongly connected which can make assembly difficult and lead to smaller contigs. A large k-mer size can recognize small repeats and therefore reduce the number of branches in the graph. On the other hand, longer k-mers can lead to a very sparse and fragmented graph which can hinder optimal assembly as well. 
...
There are a few tools available that one can use to evaluate the quality of an annotation, but in general one can look at the annotations to see if they are as expected for the species, nothing important is missing and no strange genes are added that do not make sense. Furthermore, it is advisable to compare the annotation to a curated annotation of a (related) reference genome and check if there are major differences. 
...
Mapped reads were counted with the Python package HTSeq that uses the BAM file generated in the previous step as well as the gff file produced during annotation. The histogram visualizing the distribution of counts per gene shows that the read count for most genes is actually zero which means that most genes are not expressed. In theory, a gene is expressed if the read count is > 0, so if at least one transcript is present. In order to do proper statistical analysis, this does not provide a lot of information though and it is more informative to investigate if expression is significantly different between the two conditions. In order to account for the fact that the counts for most genes are zero, the counts are normalized. The histogram of the normalized counts shows something like a normal distribution. 


### 9 Differential Expression

Differential expression analysis of the RNA sequence data retrieved from cultures grown in BH and serum was performed with the R package DESeq2. After creating the DESeqDataSet object from the read counts the DESeq2 pipeline was executed. Bevor inspecting the result table further, insignificant results were filtered out. Usually, results are understood as significant if the p-value is below a predefined threshold (often 0.05 or 0.01). In the case of differential expression analysis, there are thousands of genes involved and accepting a p-value of for example 0.05 as significant would mean a great risk of false positives. To avoid and minimize that risk, DESeq2 calculates an adjusted p-value that is corrected with the Benjamini–Hochberg procedure. This value is then used to filter significant results. In this case, removing all the results with an adjusted p-value > 0.1 resulted in a total of 664 significantly differentially expressed genes. Analyzing these results further and looking at the genes with the strongest up- or downregulation revealed a differentially expressed gene cluster involved in purine biosynthesis just as the results of the paper show as well. Genes involved in pyrimidine biosynthesis, which were mentioned in the paper as well, were not among the 50 most upregulated genes. The paper does not mention specifically how strong their expression differs, so it might as well be that they were particularly interested in these genes for other reasons and picked them out from medium strong differential expression results to do Tn-Seq analysis, which later proved their importance, because of that. Genes involved with the phosphotransferase system were found among the most upregulated genes just as in the paper. So looking at the results of this differential expression analysis, the upregulation of genes involved in purine biosynthesis indicates that *E. faecium* manages to grow in nutrient-poor human serum by metabolic adaptation. 
...
Fastq quality indicates how well supported a sequenced base is, mapping quality measures how confident we can be that a read has been aligned with the reference in the right place and variant quality estimates a likelihood for observing a call by chance.

### 13 Antibiotic Resistance Potential

Resistance to several different antibiotics is a known problem for *E. faecium* and a great concern. Analogous to the approach the authors of the paper took, ResFinder was used to discover genes involved in acquired antibiotic resistance. It found the vanHAX operon which is responsible for vancomycin resistance and macrolide resistance gene mrs(C). The dfrG gene which causes trimethoprim resistance that the authors found was not discovered here which could be due to quality differences of the assembly. ResFinder did, however, get a hit for an aminoglycoside resistance gene that was not mentioned in the paper. Then again, it is commonly known that Enterococci and especially *E. faecium* can have resistance against aminoglycosides too. In conclusion, resistance of this strain to vancomycin and a macrolide resistance seems to be established whereas the aac(6')-Ii gene involved in aminoglycoside resistance need further investigation but is very likely as well. 

## Conclusion

All in all, this project was a successful attempt to reproduce the most important results of the chosen paper. The genome assembly generated with PacBio reads is of decent quality which multiple evaluation steps proved. Sic plasmids and one chromosome were found in the assembly. The assembly was successfully annotated with a total of 3193 protein coding sequences as well as 140 signal peptides, 101 tRNA, 1 tmRNA and 23 rRNA sequences. Several antibiotic resistance genes were discovered indicating resistance against vancomycin and macrolides as well as a suspected resistance against aminoglycosides. Aquired resistance of pathogenic bacteria against common antibiotics is a severe problem and a challenge we will be faced with more and more in the future. This strain is specifically known for causing severe infections of the bloodstream in hospitalized patients, so its antibiotic resistance potential is a strong potential threat. 

RNA reads were mapped to the assembly and counted. Differential expression analysis of the samples grown in BH and human serum later revealed the existance of a strongly differentially expressed gene cluster that is involved in purine biosynthesis analogous to what the authors of the paper found. These genes, which are amongst the most upregulated in serum, were further investigated by comparing synteny with closely related *Enterococcus faecalis*. It was observed that the order of these genes is indeed conserved between the two species which confirms their functional relationship. In order to be able to prove that the proteins encoded by these genes are necessary and responsible for this strain's ability to grow in human serum and investigate them as possible targets for treatment against infections, future work should include the analysis of the Tn-Seq data. Nucleotide biosynthesis is also suggested to play a role in the microorganism's virulence. These results are already a strong indication that metabolic adaptation and speficfically purine biosynthesis is critical in *E. faecium*'s survival in serum. 
...
### References
 ...
 - Arias, C. A., & Murray, B. E. (2012). The rise of the enterococcus: Beyond vancomycin resistance. Nature Reviews. Microbiology, 10(4), 266-278. doi:10.1038/nrmicro2761